...
export const StreamChatDriver: React.FC<StreamChatDriverProps> = ({
  ...
  // Compact ChannelList component for drivers
    ...
    <div className="p-3 rounded-lg shadow-2xl flex flex-col h-full bg-slate-800">
      ...
      <Chat client={streamClient} theme="str-chat__theme-dark">
        <div className="flex-1 flex gap-2 min-h-0">
          {/* Channel list - hidden on mobile unless selector is open */}
          {(!isMobile || showChannelSelector) && (
            <>
              {isMobile && (
                <div
              ...
              <div className={`${isMobile ? 'absolute inset-0 z-10 bg-slate-800 p-4' : 'w-64'}`}>
              {isMobile && (
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-sm font-semibold text-white">Vyberte chat</h4>
                  <button
                    onClick={() => setShowChannelSelector(false)}
          ...
          {/* Main chat area - hidden on mobile when channel selector is open */}
          <div className={`flex-1 bg-slate-900/50 rounded-lg flex flex-col min-h-0 ${isMobile && showChannelSelector ? 'hidden' : ''}`}>
            {currentChannel ? (
              ...
              <div className="flex-1 flex items-center justify-center p-6">
                ...
                <p className="text-sm leading-relaxed mb-4">
                  {isMobile
                    ? 'Klepněte na tlačítko menu pro výběr chatu.'
                ...
                {isMobile && (
                  <button
                    ...
                    className="px-4 py-2 bg-[#8FBCBB] hover:bg-[#7FA8AA] text-white rounded-lg transition-colors flex items-center gap-2"
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
                    </svg>
